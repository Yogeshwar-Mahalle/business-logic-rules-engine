        ...
        for ( String line : lines ) {
            ...
            if ( line.length() > 20 ) {
                ...
                if( fieldsDataTransformMapping != null ) {
                    config = new JSONObject(fieldsDataTransformMapping.getMappingExpressionScript());
                }
    ...
    private JSONObject parseISO8583(String message, JSONObject formatMap)
    {
        ...
        if(!formatMap.equals(new JSONObject()))
        {
            ...
            if(message.length() > 20)
            {
                ...
                this.iso8583Structure = new ISO8583Structure(mtiType, formatMap);
                            ...
                            if(iso8583Structure.hasField(idx))
                            {
                                ...
                                if(ISO8583Structure.inArray(fieldFromTemplate, fieldName))
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        format = fieldFormat.get(ISO8583FieldInfo.DataElementConfig.FORMAT.getText()).toString();
                                        variable = fieldFormat.get(ISO8583FieldInfo.DataElementConfig.NAME.getText()).toString();
